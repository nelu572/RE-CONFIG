...
ROOM 05는 박스와 측면 스위치의 상호작용을 학습하는 방이다. 플레이어가 박스를 오른쪽으로 밀어 스위치를 누르면 EXIT 앞의 가로 문이 열린다.
...
1. 플레이어는 오른쪽 EXIT 앞의 닫힌 H 문을 확인한다.
2. B 박스를 오른쪽으로 밀어 T 스위치를 누른다.
3. H 문이 오른쪽으로 열리면 플레이어는 오른쪽의 E EXIT로 이동한다.
