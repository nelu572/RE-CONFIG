...
- 팔레트 값은 ROOM 01~02 인게임 테스트 기준이며, 최종 아트 확정 전까지 실제 화면 평가에 따라 조정될 수 있다.
...
현재 SETTINGS UI는 ROOM 02의 `BRICK 충돌` 학습을 기준으로 한 확정 후보 상태다. ROOM 03 이후의 새 기믹을 넣기 전에 실제 게임 화면 위에서 가독성을 최종 확인해야 한다.
...
- ROOM 02에서 BRICK에 막힌 뒤 `Z` 힌트가 나오고, SETTINGS 내부에서는 `시스템` CATEGORY와 `BRICK 충돌` SETTING을 순서대로 강조한다.
...
- 실제 ROOM 02 화면 위에 패널을 띄웠을 때 BRICK WALL, EXIT, 플레이어가 패널 뒤에서 과하게 산만하지 않은지 확인한다.
